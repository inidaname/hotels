...
export async function getCounterById (req, res) {
    try {
        ...
        const findCounter = await counterModel.findById(id).lean().populate('userId').select('-password -__v').exec();
...
export async function getCounterByUserId (req, res) {
    try {
        ...
        const findCounter = await counterModel.findOne({userId: id}).lean().populate('userId').select('-password -__v').exec();
...
export async function updateData (req, res) {
    try {
        ...
        const checkUser = await counterModel.findByIdAndUpdate(id, updates, {new: true}).lean().select('-__v').exec();
