...
export async function getGuestByRoomNumber(req, res) {
    try {
        ...
        const getRoom = await roomLogsModel.findOne({ roomNumber: number, checkedInStatus: "occupied" }).lean().select('-__v').populate('customer', '-__v').populate('room', '-__v').populate('checkedInBy', '-__v').exec();
        if (!getRoom) {
