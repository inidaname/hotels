...
export async function getGuestByRoomNumber(req, res) {
    try {
        ...
        const getRoom = await roomLogsModel.findOne({ roomNumber: number, $or: [{ checkedInStatus: "occupied" }, { checkedInStatus: "reserved" }] }).lean().select('-__v').populate('customer', '-__v').populate(['room', 'room.roomTypeId'], '-__v').populate('checkedInBy', '-__v').exec();
        if (!getRoom) {
